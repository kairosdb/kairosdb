...
public class UniqueId implements UniqueIdInterface
{
	...
	public byte[] getOrCreateId(String name) throws HBaseException
	{
		...
		while (attempt-- > 0)
		{
			...
			if (lock == null)
			{  // Should not happen.
				LOG.error("Got a null pointer as a RowLock!");
				continue;
			}
			...
			try
			{
				...
				{
					LOG.error("Unexpected exception type when assigning an ID,"
		...
		try
		{
			...
			while ((rows = scanner.nextRows().joinUninterruptibly()) != null)
			{
				for (final ArrayList<KeyValue> row : rows)
				{
					if (row.size() != 1)
					{
						LOG.error("Shouldn't happen!  Scanner " + scanner + " returned"
					...
					{
						throw new IllegalStateException("For kind=" + kind()
			...
			throws HBaseException
	{
		...
		while (attempts-- > 0)
		{
			...
			{
				LOG.error("Unexpected exception type, put=" + put, e);
			}
		}
