...
public class FilterRegistry
{
     ...
     */
    private final EventBusWithFilters bus;

    public FilterRegistry(EventBusWithFilters bus)
    {
    ...
    void register(Object filter, int priority)
    {
        ...
        for (Map.Entry<Class<?>, Collection<FilterSubscriber>> entry : listenerMethods.asMap().entrySet()) {
            ...
            CopyOnWriteArrayList<FilterSubscriber> eventFilterSubscribers = subscribers.get(eventType);

            if (eventFilterSubscribers == null) {
                SortedCopyOnWriteArrayList<FilterSubscriber> newSet = new SortedCopyOnWriteArrayList<>(COMPARATOR);
                eventFilterSubscribers = MoreObjects.firstNonNull(subscribers.putIfAbsent(eventType, newSet), newSet);
            }
            eventFilterSubscribers.addAll(eventMethodsInFilter);
        }
    }
    ...
    private static ImmutableList<Method> getAnnotatedMethodsNotCached(Class<?> clazz)
    {
        ...
        for (Class<?> supertype : supertypes) {
            for (Method method : supertype.getDeclaredMethods()) {
                if (method.isAnnotationPresent(Filter.class) && !method.isSynthetic()) {
                    ...
                    checkArgument(returnType.equals(parameterTypes[0]),
                            "Method %s must have return type of %s",
